async function createObjectViaProxy() {
  const initialData = {
    name: "Venkata Sai Developers DB v11",
    data: {
      projects: [
        {
  ...
  try {
    const res = await fetch('https://api.allorigins.win/raw?url=' + encodeURIComponent('https://api.restful-api.dev/objects'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(initialData)
    });
    const json = await res.json();
    console.log('ALLORIGINS_RES:', json);
  } catch (err) {
    console.error('ERR:', err.message);
  }
}

createObjectViaProxy();
