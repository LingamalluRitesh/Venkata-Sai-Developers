...
// Test creating a bin on jsonbin.io or jsonstorage.net
async function testJsonstorageCreate() {
  try {
    const initialData = {
      projects: [
        {
    ...
    console.log('NPOINT_STATUS:', res.status);
    const text = await res.text();
    console.log('NPOINT_BODY:', text);
  } catch (err) {
    console.error('NPOINT_ERR:', err.message);
  }
}

testJsonstorageCreate();
