...
import { WhatsAppIcon, PhoneCallIcon } from './components/Icons';
...
const MainContent: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      {/* Floating Quick Contact Buttons (Official Vector SVG WhatsApp & Phone Call Icons) */}
      <div className="fixed bottom-6 left-6 z-40 flex flex-col gap-3">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="w-14 h-14 rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all group"
          title="Chat on WhatsApp (+91 89788 15621)"
        >
          <WhatsAppIcon className="w-full h-full drop-shadow-xl group-hover:rotate-12 transition-transform" />
        </a>
        <a
          href="tel:+919030903364"
          className="w-14 h-14 rounded-2xl flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all group overflow-hidden"
          title="Call Venkata Sai Developers (+91 90309 03364)"
        >
          <PhoneCallIcon className="w-full h-full drop-shadow-xl group-hover:rotate-12 transition-transform" />
        </a>
      </div>
